...
export default class QueryMessages extends Component {
    ...
    messageToPersons = (concatMessages,me) => {
        const messages = concatMessages
        const persons = []
        const personIds = []
        const sortedMessages = messages.sort(
            (a, b) => (new Date(b.createdAt) - new Date(a.createdAt))
    ...
    personLastMessage = (concatMessages,me) => {
        const messages = concatMessages
        const personLM = {}
        const sortedMessages = messages.sort(
            (a, b) => (new Date(b.createdAt) - new Date(a.createdAt))
    ...
    concatMessages = (me, storageMessages) => {
        let newMessages
        newMessages = me.messages.sort(
            (a, b) => (new Date(b.createdAt) - new Date(a.createdAt))
        )
            ...
            <Query query={GET_ME}>
                {
                    ({ loading, error, data }) => {
                        const me = data.me
                        const persons = this.messageToPersons(concatMessages,me)
                                    ...
                                    person => (
                                        ...
                                        >
                                            ...
                                            <Body>
                                                ...
                                                <Text note numberOfLines={1}>{`${this.personLastMessage(concatMessages,me)[person.id]}`}</Text>
                                            </Body>
                                            <Right>
                                                {this.renderBadge(concatMessages, person, me)}
                                            </Right>
                                        </ListItem>
                                    )
                                )}
